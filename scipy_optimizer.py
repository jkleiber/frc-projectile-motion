...
def scipy_optimize_main(constraints: ProjectileMotionConstraints, target_info: TargetInfo, projectile: Projectile, flywheel_diameter: float, objective_fn: Callable):
    ...
    bounds = Bounds([constraints.flywheel_rps.min, constraints.launch_angle.min], [
                    constraints.flywheel_rps.max, constraints.launch_angle.max])

    x0 = [flywheel_rps, launch_angle]
